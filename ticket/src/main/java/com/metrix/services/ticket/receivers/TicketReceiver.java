    ...
    private void createTicket(TicketsForm form) {
        try {
            ...
            for (int i = 0; i < form.getNumberOfTicket(); i++) {
                Tickets newTicket = new Tickets(0, form.getOperator(), generateQRcode(), false, new Date(), null,
                        events);
                repo.save(newTicket);
            }
    ...
    private Events validate(TicketsForm form) throws MetrixException {
        ...
        if (!oEvent.isPresent()) {
            throw new MetrixException(-1,
                    String.format("TicketReceiver::validate()::Event with Id [%d] not found", form.getEventId()), "");
        } else {
            // Check whether the ticket is active and the corresponding event is start sell
            Events event = oEvent.get();
            if (!event.isStartSell()) {
                throw new MetrixException(-1, String.format(
                        "TicketReceiver::validate()::Event with Id [%d] not started yet", form.getEventId()), "");
            }
